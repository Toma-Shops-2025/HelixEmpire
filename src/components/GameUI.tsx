...
export function GameUI({
  ...
  return (
    <div className="absolute inset-0 flex flex-col justify-end text-white z-[2500] pointer-events-none">
      ...
      {/* Content Layer - FORCED pointer-events-auto */}
      <div className={cn(
        ...
        {activeTab === 'inventory' && (
          <div className="w-full max-w-md space-y-8 pointer-events-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              {SKINS.map(skin => (
                <button key={skin.id} onClick={() => { onSkinSelect(skin.id); setCurrentSkin(skin.id); }} className={cn("p-6 rounded-[32px] border-4 flex flex-col items-center gap-3 relative active:scale-95 transition-all pointer-events-auto", currentSkin === skin.id ? "border-primary bg-primary/20" : "border-white/10 bg-white/5")}>
        ...
        {activeTab === 'store' && (
          <div className="w-full max-w-md space-y-8 text-center pointer-events-auto">
            ...
            <div className="space-y-4">
                <button onClick={() => { console.log("BUY PACK"); purchase(PRODUCT_EMPIRE_PACK); }} className="w-full bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-[40px] flex justify-between items-center shadow-xl active:scale-95 transition-all pointer-events-auto">
                ...
                <button onClick={() => { console.log("BUY COINS"); purchase(PRODUCT_COINS_1000); }} className="w-full bg-white/5 border-2 border-white/10 p-6 rounded-[40px] flex justify-between items-center active:scale-95 transition-all pointer-events-auto">
        ...
        {activeTab === 'event' && (
          <div className="w-full max-w-md space-y-8 text-center pointer-events-auto">
            ...
            <div className="bg-gradient-to-br from-green-900 to-emerald-900 p-8 rounded-[50px] border-4 border-white/10 mb-8 shadow-2xl">
                ...
                <button className="w-full bg-white text-green-900 py-5 rounded-3xl font-black mt-6 active:scale-95 transition-all shadow-lg pointer-events-auto">BROWSE ITEMS</button>
            </div>
        ...
        {user && <button onClick={signOut} className="w-full py-5 bg-white/5 border-2 border-white/10 rounded-3xl font-black uppercase text-[10px] tracking-widest opacity-40 mt-8 active:scale-95 transition-all pointer-events-auto">Logout of Empire</button>}
      </div>
      ...
      <nav className={cn(
        ...
        {activeTab !== 'play' && (
            <button onClick={() => setActiveTab('play')} className="bg-primary p-4 rounded-full shadow-glow active:scale-90 transition-all pointer-events-auto">
...
function NavButton({ icon: Icon, label, active, onClick }) {
  return (
    <button onClick={onClick} className={cn("flex flex-col items-center justify-center gap-1 w-20 py-2 transition-all active:scale-90 pointer-events-auto", active ? "text-primary" : "text-white/30")}>
